...
export default async function Agenda({
  ...
  if (vista === 'mes') {
    ...
    titulo = primero.toLocaleDateString('es-ES', {
      month: 'long',
      year: 'numeric',
      timeZone: ZONA,
    });
  ...
  function Recordatorio({ cita }: { cita: CitaAgenda }) {
    ...
    return (
      <details className="mt-2">
        ...
        <p className="mt-1 rounded-lg bg-ground p-2 text-xs text-ink ring-1 ring-line">{texto}</p>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <p className="text-sm text-ink2">
            <span className="font-medium capitalize text-ink">{titulo}</span> · {visibles.length}{' '}
            cita{visibles.length === 1 ? '' : 's'}
          </p>
        </div>
      ...
      {vista !== 'rango' && (
        <div className="mt-3 flex flex-wrap items-center gap-3.5 text-xs text-ink2">
          ...
          <span className="text-muted">
            · El recordatorio discreto se envía al contacto con quien se agendó · Los terapeutas ven
