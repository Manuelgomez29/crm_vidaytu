...
function Bloque({
  ...
  return (
    <section className="rounded-xl bg-surface p-4 ring-1 ring-line">
      ...
      <form
        action={crearElementoCatalogo.bind(null, catalogo)}
          ...
          <li key={e.id}>
            ...
            >
              <input
                ...
                className={`${inputAdmin} min-w-40 flex-1`}
              />
...
export default async function AdminCatalogos({
  ...
  const normalizar = (
    filas:
      { id: string; nombre: string; slug: string; activo?: boolean; activa?: boolean }[] | null,
  ): Elemento[] =>
    (filas ?? []).map((f) => ({
      id: f.id,
      nombre: f.nombre,
