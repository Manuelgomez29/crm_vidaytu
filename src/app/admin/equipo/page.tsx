...
export default async function AdminEquipo({
  ...
  const [
    ...
    supabase
      .from('perfiles')
      .select('id, nombre, email, rol, alcance, activo, acceso_clinico')
      .order('rol')
      .order('nombre'),
    ...
    supabase
      .from('ausencias')
      .select('id, perfil_id, desde, hasta, motivo')
      .order('desde', { ascending: false }),
    supabase
      .from('objetivos')
      .select('perfil_id, mes, meta_citas, meta_conversiones, meta_ingresos'),
  ]);
  ...
  const cartera = new Map(
    (filasCartera ?? []).map((f) => [
      ...
      {
        casos: f.casos,
        tareas: f.tareas_pendientes,
        citas: f.citas_futuras,
    ...
    >
      ...
      <section className="mb-6 rounded-xl bg-surface p-4 ring-1 ring-line">
        ...
        <form action={crearUsuario} className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="flex flex-col gap-1 text-sm font-medium text-ink">
            Alcance
            <select name="alcance" defaultValue="grupo" className={inputAdmin}>
              <option value="grupo">Todo el grupo</option>
              <option value="centros">Solo los centros marcados abajo</option>
            </select>
            <span className="text-xs font-normal text-ink2">
              Solo cambia algo si el rol es dirección: admisiones y terapeuta ya están limitados por
              su rol. Una dirección «solo sus centros» manda en todo lo de esos centros —casos,
              clínica, equipo, facturación— pero no toca los catálogos, los parámetros ni el lead
              scoring, que son de todos.
            </span>
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-ink">
        ...
        <p className="mt-2 text-xs text-muted">
          La invitación es la vía recomendada. Mientras no haya SMTP propio en Supabase, los envíos
          van muy limitados: si no llega, desmarca la casilla y dale una contraseña inicial por un
          canal aparte. En ambos casos tendrá que activar la verificación en dos pasos.
        </p>
      </section>
      ...
      <section className="mb-6 rounded-xl bg-surface p-4 ring-1 ring-line">
        ...
        <p className="mb-3 text-xs text-ink2">
          Para vacaciones, bajas o salidas del equipo. Cada caso queda anotado en su historial y la
      ...
      <section className="mb-6 rounded-xl bg-surface p-4 ring-1 ring-line">
        ...
        <p className="mb-3 max-w-[80ch] text-xs text-ink2">
          Traspasa de una vez <b>todo lo vivo</b> de una persona: sus casos, sus tareas pendientes,
          sus citas futuras y sus pacientes. Lo ya hecho —tareas completadas, citas pasadas,
          sesiones— sigue diciendo quién lo hizo: eso no se toca.
        </p>
        ...
        <p className="mt-3 max-w-[80ch] text-xs text-muted">
          ...
          que aún tenga trabajo asignado, y es a propósito: es lo que evita que un caso se quede sin
          dueño sin que nadie se entere. Los pacientes solo pueden ir a alguien con acceso clínico;
          si no, dejarían de verlos todos menos dirección.
        </p>
      </section>
          ...
          return (
            ...
            >
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {/*
                    Que se vea sin abrir nada. «Direccion» a secas ya no dice
                    hasta donde llega: hay quien manda en el grupo y quien manda
                    en un centro, y es la diferencia mas importante de esta
                    pantalla.
                  */}
                {p.rol === 'direccion' && (
                  <span className={`chip ${p.alcance === 'grupo' ? 'chip-primary' : 'chip-warn'}`}>
                    {p.alcance === 'grupo'
                      ? 'Dirección de grupo'
                      : `Dirección de ${
                          susCentros
                            .map((id) => (centros ?? []).find((c) => c.id === id)?.nombre)
                            .filter(Boolean)
                            .join(' y ') || '— sin centro asignado'
                        }`}
                  </span>
                )}
                {!p.activo && (
                  ...
                  {p.rol === 'direccion' && (
                    <label className="flex flex-col gap-1 text-xs text-ink2">
                      ...
                      <select name="alcance" defaultValue={p.alcance} className={inputAdmin}>
                        <option value="grupo">Todo el grupo</option>
                        <option value="centros">Solo sus centros</option>
                      </select>
                    </label>
                  )}
                  <label className="flex items-center gap-1.5 pb-2 text-sm text-ink2">
                    ...
                    <ul className="mb-2 flex flex-col gap-1">
                      {susAusencias.map((a) => (
                        <li key={a.id} className="flex items-center justify-between gap-2 text-sm">
                          <span>
                            {fecha(`${a.desde}T12:00:00`, false)} →{' '}
                            {fecha(`${a.hasta}T12:00:00`, false)}
                            {a.motivo && ` · ${a.motivo}`}
                          </span>
